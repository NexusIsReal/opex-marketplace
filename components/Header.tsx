...
export default function Header({ fixed = true }: { fixed?: boolean }) {
  ...
  return (
    <div className={fixed ? 'fixed top-0 left-0 right-0 z-50 py-6' : 'py-6'}>
      <div className="w-[95%] max-w-[1400px] mx-auto">
        <header 
          className={`
            flex justify-between items-center px-8 py-5 rounded-3xl 
        ...
        >
          ...
          {/* Subtle border glow */}
          <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-gray-300/10 via-silver/10 to-gray-400/10 opacity-0 hover:opacity-100 transition-opacity duration-700 pointer-events-none blur-xl"></div>
          ...
          <div className="flex items-center group cursor-pointer relative z-10">
            <div className="relative mr-4">
              ...
              <div className="relative w-12 h-12 rounded-2xl backdrop-blur-xl bg-gradient-to-br from-gray-600/30 via-gray-500/25 to-gray-400/30 border border-gray-400/30 flex items-center justify-center shadow-lg group-hover:shadow-[0_0_32px_rgba(156,163,175,0.4)] transition-all duration-500">
                <span className="text-white font-bold text-xl bg-gradient-to-br from-gray-100 to-gray-300 bg-clip-text text-transparent">O</span>
                
                {/* Inner glow */}
                <div className="absolute inset-1 rounded-xl bg-gradient-to-br from-gray-300/15 to-transparent opacity-50"></div>
              </div>
              
              {/* Animated glow effect */}
              <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-gray-300 via-gray-400 to-gray-500 opacity-0 group-hover:opacity-25 blur-lg transition-all duration-500 -z-10"></div>
              
              {/* Floating particles effect */}
              <div className="absolute -top-1 -right-1 w-2 h-2 rounded-full bg-gray-300/70 opacity-0 group-hover:opacity-100 animate-pulse transition-opacity duration-500"></div>
            </div>
            
            <div className="relative">
              <h1 className="text-2xl font-bold bg-gradient-to-r from-gray-100 via-gray-200 to-gray-300 bg-clip-text text-transparent group-hover:from-gray-200 group-hover:via-gray-100 group-hover:to-white transition-all duration-500">
                OPEX
              </h1>
          ...
          {/* Enhanced Navigation */}
          <nav className="hidden lg:flex items-center space-x-10 relative z-10">
            {[
              ...
              <div key={index} className="relative group">
                <a 
                  href={item.href} 
                  className={`
                    text-sm font-semibold transition-all duration-400 relative py-3 px-4 rounded-xl
                ...
                >
                  ...
                  {item.active && (
                    <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-8 h-1 bg-gradient-to-r from-gray-300 via-gray-200 to-gray-100 rounded-full shadow-lg shadow-gray-300/30"></div>
                  )}
                  
                  {/* Hover indicator */}
                  <div className="absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-0 h-1 bg-gradient-to-r from-gray-300 to-gray-100 rounded-full opacity-0 group-hover:opacity-100 group-hover:w-6 transition-all duration-400 shadow-lg shadow-gray-300/30"></div>
                </a>
              </div>
            ))}
          </nav>
          ...
          <button 
            ...
            className="lg:hidden p-3 rounded-2xl text-gray-300 hover:text-gray-100 backdrop-blur-sm hover:bg-gray-800/[0.3] border border-transparent hover:border-gray-400/[0.15] transition-all duration-300 relative z-10"
          >
            <svg 
        ...
        <div 
          className={`
            lg:hidden mt-4 p-6 rounded-3xl backdrop-blur-2xl bg-black/[0.4] 
          ...
          <nav className="flex flex-col space-y-2 relative z-10">
            {['Dashboard', 'Hiring', 'Projects', 'Talent'].map((item, index) => (
              <a 
                key={index}
                href="#" 
                className="text-sm font-semibold text-gray-300 hover:text-gray-100 transition-all duration-400 py-3 px-4 rounded-xl hover:backdrop-blur-sm hover:bg-gray-800/[0.3] border border-transparent hover:border-gray-400/[0.15] hover:shadow-lg"
              >
                {item}
              </a>
            ))}
            
            <div className="pt-4 border-t border-gray-500/[0.15] flex flex-col space-y-3">
              <a 
                href="#" 
                className="text-sm font-semibold text-gray-300 hover:text-gray-100 transition-all duration-400 py-3 px-4 rounded-xl hover:backdrop-blur-sm hover:bg-gray-800/[0.3] border border-transparent hover:border-gray-400/[0.15] hover:shadow-lg"
              >
              ...
              <a 
                href="#" 
                className="px-6 py-3 rounded-xl bg-gradient-to-r from-gray-700 via-gray-600 to-gray-700 backdrop-blur-xl text-white text-sm font-bold text-center border border-gray-400/25 shadow-lg relative overflow-hidden group"
              >
                <div className="absolute inset-0 bg-gradient-to-br from-gray-300/20 via-transparent to-transparent opacity-60"></div>
